...
def Main():
    ...
    while(True):
        ...
        retgesture = "^______^While"

        if ret == True:
            if binaryMode == True:
                roi, retgesture = binaryMask(frame, x0, y0, width, height)

            else:
                roi, retgesture = skinMask(frame, x0, y0, width, height)
        ...
        cv2.putText(frame, str(retgesture), (fx,fy), font, 0.7,(0,255,0),2,1)
